...
export default function ButtonOnlyTracingQuest() {
  ...
  // Student name (locked after first set)
  ...
  function handleClick(label: string) {
      if (finished) return; // ignore clicks once finished
      ...
      if (label === current.answerLabel) {
        ...
        if (nextId) {
         setCurrentId(nextId);
          ...
          // no nextId → this was the final answer
          setFinished(true);
        }
      } else {
        setShake(true);
  ...
  function restart() {
    ...
    setFinished(false);
    setBursts([]);
    // optional: reshuffle layout for a fresh look
    computeLayout();
  }
  ...
  useEffect(() => {
      ...
      const id = setInterval(() => {
        spawnBurst();
     }, 450); // every 450ms, tweak as you like
    ...
    <div className={`relative min-h-screen w-full overflow-hidden bg-gradient-to-br ${TP.bg} text-white`}>
      ...
      <div className="relative z-10 flex items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-2 text-sm opacity-90">
          <button onClick={restart} className="ml-2 rounded-full bg-white/10 px-3 py-1 hover:bg-white/20">
            Restart
          </button>
          <button
      ...
      >
        <pre className="mt-3 whitespace-pre-wrap rounded-xl bg-black/30 p-4 text-sm leading-relaxed">{current.prompt}</pre>
      </motion.div>

      {/* Playfield */}
      <div
        ref={fieldRef}
        className="pointer-events-none relative z-10 mx-auto mt-4 h-[68vh] w-[96%] md:w-[1100px]"
        aria-label="button-playfield"
      >
        {!finished && positions.length === QUESTIONS.length &&
          QUESTIONS.map((q, idx) => {
  ...
  <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm">
    ...
    >
      ...
      {/* Bottom bar with Path + Student name (locked after set) */}
        ...
        {nameLocked ? (
          ...
          >
            ...
            <input
              ...
              maxLength={50}
              className="px-2 py-1 rounded-md text-black placeholder-black/60"
